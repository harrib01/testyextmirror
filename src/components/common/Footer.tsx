...
const Footer = (props: FooterProps) => {
  ...
  const facebookSVG = (
    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="30" height="30" rx="15" fill="#999999" />
      <path d="M18.2792 7.59463C18.3811 7.60618 18.4749 7.61773 18.5586 7.62859V9.16094H17.7725C17.0333 9.16094 16.4656 9.39022 16.091 9.81563C15.727 10.229 15.6103 10.7545 15.6103 11.2253V13.1044V13.6044H16.1103H18.355L18.0588 15.5H16.1103H15.6103V16V22.5H13.4231V16V15.5H12.9231H10.8334V13.6044H12.9231H13.4231V13.1044V10.8975C13.4231 9.74456 13.7704 8.91024 14.3262 8.36382C14.8837 7.81579 15.7092 7.5 16.7756 7.5C17.3074 7.5 17.8576 7.54682 18.2792 7.59463Z" stroke="black" />
    </svg>
  );
  ...
  const houzzSVG = (
    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="30" height="30" rx="15" fill="#999999" />
      <path d="M11.2333 10.7333V11.1098L11.5952 11.2139L21.9 14.1765V22.5H17.5667V17.6667V17.1667H17.0667H13.3333H12.8333V17.6667V22.5H8.5V7.5H11.2333V10.7333Z" stroke="black" />
    </svg>
  )
  ...
  const socialLinks =
    ...
    { link: props.pinterest, label: <img className="mr-3" src={pinterest} alt="pinterest" /> },
    { link: props.instagram, label: <div className="mr-3">{instaSVG}</div> },
    { link: props.houzz, label: <div className="mr-3">{houzzSVG}</div> }].filter(link => link.link);
        ...
        {isDesktopBreakpoint ? (
                ...
                {footerLinks.map((item, i) => (
                  <li key={item.label}>
                    {item.link == 'https://www.google.com' ? (
                      <Link className="Link Link--footer mb-4 md:mb-0 md:mr-4 tracking-[1.7px]" href={props.linkToLocator}>
                        {item.label}
                      </Link>
                    ) : (
                      <Link className="Link Link--footer mb-4 md:mb-0 md:mr-4 tracking-[1.7px]" cta={item} />
                    )}
                  </li>
                ))}
              </div>
              <div className="flex text-white whitespace-nowrap">
                <div className="tracking-[1.7px] text-[18px] leading-[20px] font-medium uppercase pr-8">
                  1-866-Cambria
                </div>
                <div className="flex flex-wrap">
                  {linksBottom.map((link, i) => (
                    <Link className="Link text-[12px] leading-[18px] px-2 tracking-[1.7px] font-secondary font-light border-r last:border-none hover:underline" key={i} cta={link} eventName={`bottomlink${i}`} />
                  ))}
                </div>
              </div>
            </div>
            <div>
              <div className="my-4 md:my-0 flex flex-row items-center">
                {socialLinks.map((socialLink, i) => (
                  socialLink.link
                    ? <Link className="Link Link--primary" key={i} href={socialLink.link} eventName={`social${i}`}>
                      {socialLink.label}
                    </Link>
                    : null
                ))}
              ...
              <div className="flex flex-col gap-4 pt-4 w-fit">
                {props.cta1 && (
                  <Link className="Button Button--primary uppercase w-[100%]" cta={props.cta1} eventName="cta1" />
                )}
                {props.cta2 && (
                  <Link className="Button Button--primary uppercase w-[100%]" cta={props.cta2} eventName="cta2" />
                )}
              </div>
              <div className="flex flex-row pt-4 gap-6">
                ...
                {props.googlePlay && (
                  <Link href={props.googlePlay} eventName="googlePlay">
                    <img src={playStoreIcon} alt="google play store icon" />
                  </Link>
                )}
                {props.appleStore && (
          ...
          <div className="text-white flex justify-center">
            <div className="uppercase tracking-[1.7px] font-medium flex text-[14px] underline mx-[26px]">
              <Link ng-href="tel:18662262742" target="" href="tel:18662262742" eventName="mobilePhone">
                1-866-CAMBRIA
              </Link>
            </div>
            {props.linksTop.map((item: CTA) => (
              <div key={item.label}>
                <Link className="uppercase tracking-[1.7px] font-medium flex text-[14px] underline mx-[26px]" cta={item} eventName="mobileLink" />
              </div>
            ))}
          </div>
        )}
