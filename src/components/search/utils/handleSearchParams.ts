...
export const facet_config = {
  c_designType: 0,
  c_locationType: 0,
  c_primaryFinanceByCambriaAccount: 0,
  c_features: 0,
  c_cambriaType: 0,
  c_webType: 0,
  c_locationStatus: 0,
...
export function useLoadInitialSearchParams(
  ...
  useEffect(() => {
    const loadUrlParams = async () => {
      ...
      // Load facets
      const facetParams = Array.from(searchParams.entries()).filter(
        ([fieldId, _]) =>
          Array.from(Object.keys(facet_config)).includes(fieldId)
      );
      ...
      const locationFilterId =
        LOCATOR_STATIC_FILTER_FIELD === "builtin.location"
          ? filterType
            ? locationTypeToFilter(filterType)
            : LOCATOR_STATIC_FILTER_FIELD
          : LOCATOR_STATIC_FILTER_FIELD;
      ...
      if (locationPlaceId) {
        ...
        if (locationDisplayName === "My Location") {
          ...
          if (parseFloat(lat) && parseFloat(lng)) {
            ...
            console.error(
              "Invalid 'q' param, must be of form `${lat},${lng}`."
            );
          }
        }
...
export function useUpdateFacetParams(
  ...
  useEffect(() => {
    ...
    if (facetFilters) {
      const activeFacets = facetFilters.filter(
