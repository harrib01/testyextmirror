...
export default function SearchBox(props: SearchBoxProps) {
  ...
  const pin1SVG = (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12.0407 1.10864C14.1921 1.10864 16.1398 1.98065 17.5496 3.3905C18.9595 4.80034 19.8315 6.74803 19.8315 8.89938C19.8315 11.8038 17.2159 17.2049 12.0403 25.1086C6.86528 17.2043 4.25 11.8036 4.25 8.89938C4.25 6.74803 5.12201 4.80034 6.53185 3.3905C7.9417 1.98065 9.88938 1.10864 12.0407 1.10864Z"
        fill="#C59617"
        stroke="#4C4C4C"
      />
    </svg>
  ...
  const mapPins = [
    {
      text: "Cambria Gallery / Showroom",
      label: <span className="mr-3">{pin2SVG}</span>,
    },
    { text: "Premier Partner", label: <span className="mr-3">{pin1SVG}</span> },
    {
      text: "Authorized Sellers",
      label: <span className="mr-3">{pin3SVG}</span>,
    },
  ];
    ...
    <div className="border-b border-brand-gray-200 p-6">
      ...
      <div className="flex items-center">
        <div className="relative w-full h-[54px] text-brand-gray-500 border border-brand-gray-200 flex items-center bg-white pr-2">
          <FilterSearch
            ...
            onSelect={({
              ...
              const matchingFilters =
                searchActions.state.filters.static?.filter(
                  ({ filter, selected }) =>
                    selected &&
                    filter.kind === "fieldValue" &&
                    (LOCATOR_STATIC_FILTER_FIELD === "builtin.location"
                      ? checkIsLocationFilter(filter)
                      : searchFields.some(
                          (s) => s.fieldApiName === filter.fieldId
                        ))
                ) ?? [];
              matchingFilters.forEach((f) =>
              ...
              let datalayerURL =
                "?q=" +
                newFilter.value.toString() +
                "&qp=" +
                newDisplayName.replaceAll(" ", "+").replaceAll(",", "%2C");
              ...
              addToDatalayer({
                ...
                "dealer-locator_search_path": isProduction(
                  window.location.hostname
                )
                  ? "https://www.cambriausa.com/dealer-locator" + datalayerURL
          ...
          />
          <span className="m-3 mr-2 w-4">{searchIcon}</span>
        </div>
        <GeolocateButton
          className="ml-4"
      ...
      {showPins && (
        <div className="pt-6 flex flex-wrap gap-4">
          ...
          <div className="relative">
            <div
              className="flex min-w-[130px] md:min-w-[160px]"
            ...
            <div
              ...
              Meet with a Cambria expert to view full size slabs, collaborate on
              your project, and discuss next steps in the purchasing process.
            </div>
          </div>
          <div className="relative">
            <div
            ...
            <div
              ...
              Our Premier Partners are dedicated to providing you the best
              Cambria experience from start to finish.
            </div>
          </div>
          <div className="flex min-w-[130px] md:min-w-[160px]">
            {mapPins[2].label}
