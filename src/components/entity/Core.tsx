...
const CoreHeading = (props: { children: React.ReactNode }) => {
  return <h2 className="Heading Heading--sub mb-4 font-medium uppercase font-primary tracking-[1.7px]">{props.children}</h2>
}
...
const Core = (props: CoreProps) => {
  ...
  let isClosed = false;
  if (hours && monday?.isClosed && tuesday?.isClosed && wednesday?.isClosed && thursday?.isClosed && friday?.isClosed && saturday?.isClosed && sunday?.isClosed) {
    isClosed = true;
  }
  ...
  return (
    ...
    >
      {profile.c_heroImage && (
        <Image
          className={c(
            { "Core-image w-[100%] h-[780px] md:h-[700px] lg:h-auto lg:max-h-[650px]": !profile.c_cambriaType || profile.c_cambriaType != 'PREMIER_PARTNER' },
            { "Core-image w-[100%] h-[1400px] sm:h-[1300px] lg:h-[900px] lg:max-h-[800px]": profile.c_cambriaType == 'PREMIER_PARTNER' })}
          image={profile.c_heroImage} />
      )}
      <div
        className={c("left-0 right-0 absolute",
        ...
        <div className="container max-w-none xl:max-w-[1500px] lg:flex">
            ...
            <div className="flex flex-row flex-wrap lg:flex-nowrap pt-3 sm:pt-6 sm:border-t sm:border-[#999]">
              <div className="w-full sm:w-1/2 lg:w-[30%] md:gap-3 mb-8">
                ...
                <div className="flex flex-col">
                  <Link className="Link--primary Link--underline font-medium mb-[20px]" href={profile.c_googleListingURL ? profile.c_googleListingURL : `${getDirections(profile.address, profile.ref_listings, profile.googlePlaceId)}`} eventName="direction"
                  ...
                  {profile.mainPhone && (
                    <Link href={'tel:' + props.profile.mainPhone} className="Link--primary Link--underline font-medium" eventName="phone"
                      onClick={() => (addToDatalayer(
                        {
              ...
              {profile.c_cambriaType == 'PREMIER_PARTNER' ? (
                <div className="w-full sm:w-1/2 lg:w-[40%] md:gap-3 mb-8 flex flex-col md:items-center">
                  ...
                  {profile.c_partnerWebsite && (
                    <div className="mt-4">
                      <Link className="Link Link--primary uppercase underline hover:no-underline" href={profile.c_partnerWebsite} target="_blank" eventName="viewSite"
                        onClick={() => (addToDatalayer(
                          {
                ...
                <div>
                  {profile.c_cambriaType && profile.c_cambriaType != 'HOME_DEPOT' && (
                    <div className="w-full sm:w-1/2 lg:w-[30%] md:gap-3 mb-8 whitespace-nowrap">
                      ...
                      <div className="flex flex-col">
                        <Link className="Link Link--primary uppercase underline hover:no-underline mb-4" href={profile.c_salesforceCommunitiesSchedulerURL ? profile.c_salesforceCommunitiesSchedulerURL : 'https://www.cambriausa.com/cambria-consultation'} eventName="showroomConsultation">
          ...
          {!profile.c_cambriaType || profile.c_cambriaType == 'PREMIER_PARTNER' && (
            <div className="pt-8 lg:pt-0 lg:w-[37%]">
              <div className="bg-white py-3">
                ...
                {profile.mainPhone && (
                  <div className="text-center font-secondary text-[14px]">
                    ...
                    <Link href={'tel:' + props.profile.mainPhone} className="Link--primary Link--underline" eventName="phone"
                      onClick={() => (addToDatalayer(
                        {
              ...
              <iframe
                className="bg-white"
                src={profile.c_cRMID ? props.iframe + '?Salesforce_Web_Locator_Id=' + profile.id + '&Salesforce_Account_Id=' + profile.c_cRMID : props.iframe + '?Salesforce_Web_Locator_Id=' + profile.id}
