...
- For this repo, run `yext init 3836926`

`npx pages build` or `npm run build`
- Runs a production build against your `localData`
- ***If this is your first time serving this site, you'll need to run this*** in order to populate the `sites-config/features.json` file with the stream definitions. The next command relies upon in order to fetch test data which is required to serve the site

`yext sites generate-test-data`
- Pulls an example set of `localData` from your account using the stream definitions found in `sites-config/features.json`
- This can be run with the `--entityIds` flag to specify exact IDs to pull in for local serving

`npx pages dev` - runs your code against a local dev server using Vite
